...
export default function PaletteFinder() {
  ...
  async function subscribeToKlaviyo(emailAddr) {
    ...
    // ── 1. Subscribe to list (legacy AJAX endpoint — browser safe) ──
    try {
    ...
    // ── 2. Track "Palette Selected" event (stacks on every visit) ──
    try {
      const eventProps = {
        ...customProps,
      ...
      // Klaviyo client-side event tracking via their JS snippet approach
      const payload = {
        token:  KLAVIYO_KEY,
        event:  "Palette Selected",
        customer_properties: { $email: emailAddr },
        properties: eventProps,
        time: Math.floor(Date.now() / 1000),
      };

      // Base64 encode the payload as Klaviyo's track endpoint expects
      const encoded = btoa(JSON.stringify(payload));
      await fetch(
        `https://a.klaviyo.com/api/track?data=${encodeURIComponent(encoded)}`,
        { method: "GET" }
      );
    } catch(e) {
      console.error("Klaviyo track error:", e);
    }
  }
