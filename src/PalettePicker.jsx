...
export default function PaletteFinder() {
  ...
  async function subscribeToKlaviyo(emailAddr) {
    ...
    // Build tags based on path
    const properties = {
      palette_name:   activePalette?.name ?? "",
      palette_path:   mode === "mood" ? "mood" : "image",
    };
    if (mode === "mood") {
      properties.mood_state  = q1Label;
      properties.color_temp  = q2Label;
      properties.mood_id     = q1 ?? "";
      properties.color_id    = q2 ?? "";
    } else {
      properties.palette_path = "image";
    }
    // Add DMC thread numbers as a property
    if (activePalette?.colors) {
      properties.dmc_colors = activePalette.colors.map(c => `DMC ${c.dmc}`).join(", ");
    }
